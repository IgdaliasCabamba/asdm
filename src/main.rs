...
fn main(){
    ...
    let scripts:Vec<utils::Scripts> = serde_json::from_reader(
        utils::settings::get_scripts_index(config.scripts_path)
    )
    ...
    for script in scripts{
        if name.to_owned() == script.command{
            ...
            println!("{min_size} {:?}", script.args);
            if min_size >= script.args.try_into().unwrap(){
                core::new_process(&script.path, &args); 
            }
            else{
